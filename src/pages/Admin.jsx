...
function elStyle(el, scale = 1) {
  return {
    color:      el.color  || '#1a1a1a',
    fontFamily: el.font   || 'inherit',
    fontSize:   `${Math.round((el.size||16) * scale)}px`,
...
function Handle({ pos, onMouseDown }) {
  ...
  return (
    <div className={`${base} ${styles[pos]}`}
      onMouseDown={e => { e.stopPropagation(); e.preventDefault(); onMouseDown(pos) }}/>
  )
}

/* ─── TEXT CANVAS ─── */
function TextCanvas({ image, elements, setElements, height = 280, actualHeight = null }) {
  const [selectedId, setSelectedId] = useState(null)
  const containerRef = useRef(null)
  const moving       = useRef(null)   // { id }
  const resizing     = useRef(null)   // { id, handle, startX, startY, startSize, startWidth }
  const selected     = elements.find(e => e.id === selectedId)
  const scale        = actualHeight ? height / actualHeight : 1
  ...
  const getPct = useCallback((e) => {
    ...
    return {
      x: Math.max(0, Math.min(90, Math.round(((cx - rect.left) / rect.width)  * 100))),
      y: Math.max(0, Math.min(90, Math.round(((cy - rect.top)  / rect.height) * 100))),
      rawX: cx, rawY: cy,
    ...
    if (resizing.current) {
      ...
      setElements(prev => prev.map(el => {
        ...
        if (['e','ne','se'].includes(handle)) {
          updated.width = Math.max(5, Math.min(95, Math.round(startWidth + dx)))
        } else if (['w','nw','sw'].includes(handle)) {
          updated.width = Math.max(5, Math.min(95, Math.round(startWidth - dx)))
        }

        // Size handles (corners + s)
        if (['se','sw','ne','nw','s'].includes(handle)) {
          const delta = ['ne','nw'].includes(handle) ? -dy : dy
          updated.size = Math.max(6, Math.min(300, Math.round(startSize + delta * 1.5)))
        }
  ...
  return (
    <div>
      <p className="text-sm font-medium text-gray-700 mb-2">
        👆 Canvas — <span className="text-blue-600 font-semibold">clic para seleccionar</span>, arrastra para mover,
        <span className="text-purple-600 font-semibold"> handles para redimensionar</span>
        {actualHeight && <span className="ml-2 text-xs text-gray-400">(escala 1:{Math.round(actualHeight/height*10)/10})</span>}
      </p>
      ...
      <div ref={containerRef}
        ...
        {elements.map(el => {
          ...
          const w = el.width || 40
          const preview = elStyle(el, scale)

          return (
            <div key={el.id}
              className={`absolute ${isSelected ? 'outline outline-2 outline-blue-500' : 'hover:outline hover:outline-dashed hover:outline-gray-400 hover:outline-1'}`}
              style={{ left:`${el.x}%`, top:`${el.y}%`, width:`${w}%`, zIndex:10, cursor:'move' }}
              onMouseDown={e => {
                e.stopPropagation()
                setSelectedId(el.id)
                moving.current = el.id
              }}
              onClick={e => e.stopPropagation()}>

              {el.id === 'btn'
                ? <span className="inline-block bg-gray-900 text-white px-4 py-1.5 uppercase" style={{...preview,color:'#fff'}}>{el.text||'Botón'}</span>
                : <span style={preview}>{el.text || <span className="text-gray-400 italic text-xs">{el.label}</span>}</span>
              }
              ...
              {isSelected && ['nw','n','ne','e','se','s','sw','w'].map(pos => (
                <Handle key={pos} pos={pos} onMouseDown={handle => {
                  ...
                  resizing.current = {
                    id: el.id,
                    handle,
                    startX: rect.left,  // will be overridden in first mousemove
                    startY: rect.top,
                    startSize:  el.size  || 16,
                    startWidth: el.width || 40,
                  }
                  // Capture actual mouse position at the moment of mousedown
                  const capture = (ev) => {
                    resizing.current.startX = ev.clientX
                    resizing.current.startY = ev.clientY
                    window.removeEventListener('mousemove', capture, {once:true})
                  }
                  window.addEventListener('mousemove', capture, {once:true})
                  moving.current = null
                }}/>
              ))}
              ...
              {isSelected && (
                <div className="absolute -top-5 left-0 bg-blue-600 text-white text-xs px-1.5 py-0.5 rounded whitespace-nowrap pointer-events-none">
                  {el.label} — {w}% ancho · {el.size||16}px
                </div>
              )}
            </div>
          )
        ...
        <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <textarea value={selected.text||''} onChange={e=>updateEl(selected.id,{text:e.target.value})} rows={3} className={`${inp} w-full resize-none`}/>
            </div>
            {selected.id==='btn' && (
              <div className="col-span-2">
                <label className="block text-xs text-gray-600 mb-1">URL del botón</label>
                <input value={selected.url||''} onChange={e=>updateEl(selected.id,{url:e.target.value})} className={`${inp} w-full`} placeholder="/shop"/>
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <input type="number" min="6" max="300" value={selected.size||16}
                onChange={e=>updateEl(selected.id,{size:parseInt(e.target.value)||16})} className={`${inp} w-full`}/>
            </div>
            <div>
              <label className="block text-xs text-gray-600 mb-1">Ancho (%)</label>
              <input type="number" min="5" max="95" value={selected.width||40}
                onChange={e=>updateEl(selected.id,{width:parseInt(e.target.value)||40})} className={`${inp} w-full`}/>
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <div className="flex gap-2 flex-wrap">
                <button type="button" onClick={()=>updateEl(selected.id,{bold:!selected.bold})}
                  className={`px-3 py-1.5 text-sm font-bold rounded border transition ${selected.bold?'bg-gray-900 text-white':'bg-white text-gray-700 border-gray-300'}`}>B</button>
                <button type="button" onClick={()=>updateEl(selected.id,{italic:!selected.italic})}
                  className={`px-3 py-1.5 text-sm italic rounded border transition ${selected.italic?'bg-gray-900 text-white':'bg-white text-gray-700 border-gray-300'}`}>I</button>
...
function Admin() {
  ...
  useEffect(() => { if(!editSlideId)     setSlideElements(defaultSlideEls(slideForm))    }, [slideForm.title, slideForm.subtitle, slideForm.description, slideForm.button_text])
  useEffect(() => { if(!editBrowseId)    setBrowseElements(defaultBrowseEls(browseForm)) }, [browseForm.title, browseForm.description, browseForm.button_text])
  useEffect(() => { if(!editShopBrandId) setShopElements(defaultShopEls(shopBrandForm))  }, [shopBrandForm.title, shopBrandForm.description, shopBrandForm.button_text])
  ...
  const handleSaveSlide = async e => {
    e.preventDefault()
    await saveItem(`${API}/api/slides`, {...slideForm, elements: slideElements}, editSlideId, ()=>{
      showMessage(editSlideId?'Slide updated ✅':'Slide added ✅')
      setSlideForm(EMPTY_SLIDE); setSlideElements([]); setEditSlideId(null); fetchSlides()
    })
  }
  const handleSaveBrowse = async e => {
    e.preventDefault()
    await saveItem(`${API}/api/brands`, {...browseForm, elements: browseElements}, editBrowseId, ()=>{
  ...
  const handleSaveShopBrand = async e => {
    e.preventDefault()
    await saveItem(`${API}/api/shop-brands`, {...shopBrandForm, elements: shopElements}, editShopBrandId, ()=>{
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div>
          <div className="bg-gray-50 rounded-lg p-6 mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              {editSlideId&&<button onClick={()=>{setSlideForm(EMPTY_SLIDE);setSlideElements([]);setEditSlideId(null)}} className={cancelBtn}>✕ Cancel</button>}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <TextCanvas image={slideForm.image} elements={slideElements.length?slideElements:defaultSlideEls(slideForm)} setElements={setSlideElements} height={280} actualHeight={600}/>
            </div>
            <button onClick={handleSaveSlide} disabled={uploading||(!slideForm.image&&!editSlideId)} className="mt-6 w-full bg-blue-600 text-white py-3 rounded font-semibold hover:bg-blue-700 transition disabled:opacity-50">
            ...
            {slides.map(s=>(
              <div key={s.id} className={`bg-white rounded-lg shadow overflow-hidden ${editSlideId===s.id?'ring-2 ring-blue-400':''}`}>
                <div className="relative h-40 bg-gray-100">
                  ...
                  {(s.elements||[]).map(el=>el.text&&(
                    <span key={el.id} className="absolute text-xs" style={{left:`${el.x}%`,top:`${el.y}%`,color:el.color||'#1a1a1a',fontWeight:el.bold?'bold':'normal'}}>{el.text}</span>
                  ))}
                </div>
                <div className="p-3 flex items-center justify-between">
                  <p className="font-bold text-sm text-gray-800">{s.title} {s.subtitle}</p>
                  <div className="flex gap-2">
                    <button onClick={()=>{setSlideForm({image:s.image,title:s.title||'',subtitle:s.subtitle||'',description:s.description||'',button_text:s.button_text||'',button_url:s.button_url||'',sort_order:s.sort_order||0});setSlideElements(s.elements&&s.elements.length?s.elements:defaultSlideEls(s));setEditSlideId(s.id);window.scrollTo({top:0,behavior:'smooth'})}} className={editBtn}>✏️ Edit</button>
        ...
        <div>
          ...
          <div className="bg-gray-50 rounded-lg p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <TextCanvas image={browseForm.image} elements={browseElements.length?browseElements:defaultBrowseEls(browseForm)} setElements={setBrowseElements} height={208} actualHeight={208}/>
            </div>
            ...
            {brandBanners.map(b=>(
              <div key={b.id} className={`bg-white rounded-lg shadow overflow-hidden ${editBrowseId===b.id?'ring-2 ring-blue-400':''}`}>
                ...
                <div className="p-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={()=>{setBrowseForm({image:b.image,title:b.title||'',description:b.description||'',button_text:b.button_text||'',button_url:b.button_url||'',sort_order:b.sort_order||0});setBrowseElements(b.elements&&b.elements.length?b.elements:defaultBrowseEls(b));setEditBrowseId(b.id);window.scrollTo({top:0,behavior:'smooth'})}} className={editBtn}>✏️ Edit</button>
      ...
      {activeTab==='shopbrands' && (
        <div>
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-5 py-3 mb-6 text-sm text-blue-700">🛍 Grid en <strong>/brands</strong>.</div>
          <div className="bg-gray-50 rounded-lg p-6 mb-8">
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <TextCanvas image={shopBrandForm.image} elements={shopElements.length?shopElements:defaultShopEls(shopBrandForm)} setElements={setShopElements} height={(shopBrandForm.size==='tall'||shopBrandForm.size==='big')?520:260} actualHeight={260}/>
            </div>
